...
# 근무표를 편성한다. 그렇다고 직원 권한을 통째로 주면 대타 승인·공고 관리까지
# 함께 열린다. 그래서 토큰의 role은 student 그대로 두고, 근무표 편성 경로에만
# 통하는 권한을 따로 둔다.
#
# 열리는 것: 생성 · draft 조회/편집 · 확정 · 검토 챗봇 · 배치 검증 · 부서 수합 조회
#            · 부서 정책 조회/변경(챗봇 가중치 저장 포함)
# 닫히는 것: 대타 승인/반려 · 공고/지원서 관리 · 학생 활동기간 수정
#            · 지원서 가능시간 연동
#
# 부서 정책은 처음엔 조회만 열었다가 변경까지 열었다 — 개관 시간·근무 슬롯·배정
# 인원이 곧 편성 결과라, 편성만 맡기고 기준값은 직원 몫으로 두면 팀장이 근무표를
# 짤 때마다 직원 응답을 기다리게 된다.
