...
export default function AdminSubstitutePage() {
  ...
  if (stage === 'search' && sel) {
    return (
      <AdminShell activeMenu="substitute">
        ...
        <h1 style={{ margin: '14px 0 4px', fontSize: 'var(--fs-h2)', fontWeight: 800, color: 'var(--text-strong)' }}>대타 요청 상세</h1>
        <p style={{ margin: '0 0 20px', fontSize: 'var(--fs-body)', color: 'var(--text-muted)' }}>후보 탐색과 수락은 학생이 직접 합니다. 여기서는 진행 상황을 확인하고, 후보가 수락한 요청만 최종 승인합니다.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 18, alignItems: 'start' }}>
          <AdminPanel title="요청 정보">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, fontSize: 'var(--fs-body)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, padding: 12, background: 'var(--neutral-25)', borderRadius: 'var(--radius-lg)', marginTop: 4, fontSize: 'var(--fs-sm)', color: 'var(--text-muted)', lineHeight: 1.5 }}>
            ...
            {sel.status === '수락' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ border: '1px solid var(--sogang-red)', background: 'var(--saint-row-hover)', borderRadius: 'var(--radius-lg)', padding: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 'var(--fs-title)', fontWeight: 700, color: 'var(--text-strong)' }}>{sel.substitute_name ?? sel.substitute_id}</div>
                    <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-muted)', marginTop: 2 }}>이 요청을 수락했습니다. 승인하면 근무표가 즉시 교체됩니다.</div>
                  </div>
                </div>
                {approveError && <p style={{ margin: 0, fontSize: 'var(--fs-sm)', color: 'var(--danger)' }}>{approveError}</p>}
              ...
              <p style={{ margin: 0, fontSize: 'var(--fs-body)', color: 'var(--danger)' }}>{candidatesError}</p>
            ) : !candidates ? (
              <p style={{ margin: 0, fontSize: 'var(--fs-body)', color: 'var(--text-muted)' }}>불러오는 중...</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', gap: 8, padding: 12, background: 'var(--warning-50)', borderRadius: 'var(--radius-lg)', fontSize: 'var(--fs-sm)', color: 'var(--text-muted)', lineHeight: 1.5 }}>
                  아직 수락한 후보가 없습니다. 아래 후보 중 한 명이 요청을 수락하면 이 화면에서 승인할 수 있습니다.
                </div>
                {candidates.length === 0 ? (
                  <p style={{ margin: 0, fontSize: 'var(--fs-body)', color: 'var(--text-subtle)' }}>해당 시간대에 가능한 후보가 없습니다.</p>
                ) : candidates.map(c => (
                  <div key={c.student_id} style={{ border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
                    <span style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--neutral-100)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><User size={18} color="var(--text-muted)" /></span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 'var(--fs-title)', fontWeight: 700, color: 'var(--text-strong)' }}>{c.name ?? c.student_id}</div>
                      <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-subtle)' }}>{c.student_id}</div>
                    </div>
                  </div>
                ))}
              </div>
  ...
  if (stage === 'done') {
    ...
    return (
      <AdminShell activeMenu="substitute">
        <h1 style={{ margin: '0 0 20px', fontSize: 'var(--fs-h2)', fontWeight: 800, color: 'var(--text-strong)' }}>{approved ? '대타 승인 완료' : '대타 요청 반려'}</h1>
        <AdminPanel>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '30px 0' }}>
            ...
            <h2 style={{ margin: '0 0 8px', fontSize: 'var(--fs-h2)', fontWeight: 800, color: 'var(--text-strong)' }}>
              {approved ? '대타 신청이 승인되었습니다' : '대타 요청이 반려되었습니다'}
            </h2>
            <p style={{ margin: '0 0 20px', fontSize: 'var(--fs-body)', color: 'var(--text-muted)' }}>
              {approved ? (
                <>
        ...
        <>
          ...
          <div style={{ border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-xl)', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {requests.length === 0 ? (
                  <tr><td colSpan={7} style={{ padding: '24px 16px', textAlign: 'center', fontSize: 'var(--fs-body)', color: 'var(--text-muted)' }}>대타 요청이 없습니다.</td></tr>
                ) : requests.map(r => (
                  <tr key={r.request_id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                    <td style={{ padding: '13px 16px' }}><div style={{ fontSize: 'var(--fs-body)', fontWeight: 700, color: 'var(--text-strong)' }}>{r.requester_name ?? r.requester_id}</div><div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-subtle)' }}>{r.department_name}</div></td>
                    <td style={{ padding: '13px 16px', textAlign: 'center', fontSize: 'var(--fs-body)' }}>{formatDate(r.date)}</td>
                    <td style={{ padding: '13px 16px', textAlign: 'center', fontSize: 'var(--fs-body)' }}>{r.start_time?.slice(0, 5)}-{r.end_time?.slice(0, 5)}</td>
                    ...
                    <td style={{ padding: '13px 16px', textAlign: 'center', fontSize: 'var(--fs-body)', color: 'var(--text-subtle)' }}>{formatDate(r.requested_at)}</td>
                    <td style={{ padding: '13px 16px', textAlign: 'center' }}><StatusPill status={adminStatusSlug(r.status)} label={r.status} /></td>
                    <td style={{ padding: '13px 16px', textAlign: 'center' }}>
                      {r.status === '승인'
                        ? <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-subtle)' }}>{r.approver_name ?? r.approved_by} 처리</span>
                        : r.status === '반려'
                          ? <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-subtle)' }}>반려됨</span>
