...
const th = { padding: '9px 10px', fontSize: 'var(--fs-sm)', fontWeight: 700, color: 'var(--saint-maroon)', background: 'var(--saint-tan)', textAlign: 'left', whiteSpace: 'nowrap' }
...
export function RowTable({ columns, rows, onChange, onRemove }) {
  if (rows.length === 0) {
    return <div style={{ fontSize: 'var(--fs-body)', color: 'var(--text-subtle)', padding: '10px 2px' }}>아직 추가된 항목이 없습니다.</div>
  }
  return (
              ...
              <td key={c.key ?? c.keys.join('-')} style={td}>
                {c.type === 'select' ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    <span style={{ color: 'var(--text-subtle)', fontSize: 'var(--fs-sm)', flexShrink: 0 }}>~</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
...
const tdReadOnly = { padding: '10px', borderBottom: '1px solid var(--border-subtle)', fontSize: 'var(--fs-body)', color: 'var(--text-body)' }

export function ReadOnlyRowTable({ columns, rows }) {
  if (!rows || rows.length === 0) {
    return <div style={{ fontSize: 'var(--fs-body)', color: 'var(--text-subtle)', padding: '10px 2px' }}>등록된 내역이 없습니다.</div>
  }
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--sogang-red)',
