    ...
    >
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface-card)', borderRadius: 14, width: 460, maxWidth: 'calc(100vw - 48px)', padding: 24, boxShadow: '0 20px 50px rgba(16,24,40,.25)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          <h3 style={{ margin: 0, fontSize: 'var(--fs-h3)', fontWeight: 800, color: 'var(--text-strong)' }}>
        ...
        <p style={{ margin: '0 0 18px', fontSize: 'var(--fs-body)', color: 'var(--text-muted)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {subs.map(s => (
            <div key={s.request_id} style={{ border: '1px solid var(--border-subtle)', borderRadius: 10, padding: 14 }}>
              <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-subtle)', marginBottom: 8 }}>
                {hhmm(s.start_time)}–{hhmm(s.end_time)}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 'var(--fs-body)', fontWeight: 700, color: 'var(--text-strong)' }}>
