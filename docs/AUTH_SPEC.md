...
## 6. 프론트엔드의 가드는 편의이지 권한이 아니다

**라우트 정의(`App.jsx`)에는 가드가 없습니다.** 21개 경로가 조건 없이 등록돼 있습니다.
가드는 그 아래 **레이아웃 셸**에 있습니다.

| 셸 | 가드 |
|---|---|
| `Shell.jsx` (학생 화면 10개) | 세션 없으면 `/login`, 직원이면 `/admin/posts`로 보내고 `return null` |
| `AdminShell.jsx` (관리자 화면 7개) | 세션 없으면 `/login`, `canUseAdmin`(직원 또는 학생팀장)이 아니면 `/posts`로 보내고 `return null`. 학생팀장은 `TEAM_LEAD_MENUS`(근무표 편성·부서 설정) 밖 화면이면 `return null` |
| 없음 | `LoginPage`(가드 불필요), `SaintHomePage`(세션 없으면 `/login`만 확인) |

그래서 학생이 주소창에 `/admin/posts`를 쳐도 화면이 그려지지 않고 `/posts`로 되돌아옵니다.

**하지만 이건 권한 제어가 아니라 사용자 편의입니다.** 셸의 판단 근거는 `sessionStorage`의
`stream_user`이고, 그 값은 브라우저 안에 있어 사용자가 고칠 수 있습니다. `is_team_lead`를
직접 `true`로 바꾸면 편성 화면이 그려집니다 — 그러나 그 화면이 부르는 API가 전부 3절의
게이트를 지나므로 **데이터는 403만 돌아옵니다.** 권한의 진실은 서버에만 있습니다.
