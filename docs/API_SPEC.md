...
#### `GET /api/class-time/terms`

수강 학기 목록을 조회한다. (학생 전용, REQ-SCHED-015)

수업 시간표는 **학기마다 다르므로** 모든 수업 시간 조회·저장이 학기(term) 단위로 이루어집니다. 학기 구간은 학사일정([sogang.ac.kr/ko/academic-support/calendar](https://www.sogang.ac.kr/ko/academic-support/calendar))을 옮긴 `academic_calendar_<연도>.json`의 `terms`에서 옵니다 — 정규학기는 개강일~학기말시험 종료일, 계절학기는 계절수업 개강~종강입니다.

| 항목 | 내용 |
| --- | --- |
| 인증 | 필요 (학생만) |
| Response 200 | `{ "terms": [{ "key": "2026-1", "label": "2026 봄학기", "start": "2026-03-03", "end": "2026-06-22", "current": false }, { "key": "2026-summer", ... }, { "key": "2026-2", ... }, { "key": "2026-winter", "end": "2027-01-14", ... }], "default_term": "2026-2" }` |

- `current`: 오늘이 그 학기 안이면 true. 학기 사이 방학이면 `current`인 학기가 없습니다.
- `default_term`: 화면이 기본으로 열어 둘 학기. 방학이면 **다가오는 학기**입니다 (수강신청 직후 다음 학기 시간표를 미리 짜는 흐름).
