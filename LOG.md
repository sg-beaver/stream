...
- **Before**: 근로 학생 9명 · 1주 생성. capacity 138인시 < 개관 수요(최대 150인시)라 솔버가 100% 소진 → **전원 여력 +0.0h**, 대타 요청 **0건**.
...
- **After**: 대타 요청 **4건** 복구. `OPTIMAL` solve 5.84s, 근무 55건, 개관 153.0h 중 **306/306 슬롯 100%** 최소 인원 충족, 배정 269.5인시, 솔버 배정 제약 위반 **0건**. 데모 흐름을 실제로 눌러 확인 — ①대기 후보 조회 정상, ②수락 건 승인 **200**이고 확정본 critical **0 → 0건**(승인해도 위반이 안 생긴다), ③승인은 이미 반영된 상태, ④반려는 사유 표시. 백엔드 417 passed.
...
- **Before**: `generate` **200** `OPTIMAL` 배정 30건 → `confirm` **400 "해당 학생은 부서 운영 상한 주 15시간을 초과합니다."** 국가 학생 2명이 각 20.0h(법정 상한)로 배정돼 부서 상한 15h를 넘었다. 총 138.0인시. **부서 2에서 생성 → 확정 플로우가 완결되지 않는 상태.** 예전 시드가 확정 배치를 DB에 직접 넣어 이 경로를 타지 않아 가려져 있었다.
