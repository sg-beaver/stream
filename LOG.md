...
- **After**: 왕복 불일치 **6칸 → 0칸**. 새 부서로 근무표를 생성하니 `OPTIMAL`, solve 2.24s, objective 4,876. 개관 슬롯 306개(153.0h) 중 **303개(99.0%)** 최소 인원 충족, 총 배정 279.0인시(10명, 18.5~39.5h). 미충원 3슬롯은 09/01(화) 15:00~16:30 하나뿐이고 **후보 0명** — 그 시간에 10명 전원이 수업이라 시트 자체의 성질이지 시드 결함이 아니다(전사본에서 직접 확인). 백엔드 테스트 382 passed(실 LLM 호출 2개 파일 제외).
